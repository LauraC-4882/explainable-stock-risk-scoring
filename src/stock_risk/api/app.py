...
from ..market_history import overlay_events
...
_mock_history_events: Optional[dict] = None
if MOCK_MODE:
    ...
    # [G8] Generated by scripts/build_mock_history_events.py from the real
    # curated event table plus a synthetic TSLA-shaped close series, so the
    # screenshot harness exercises every render branch (full / partial /
    # no coverage) without a "max"-period network fetch.
    _mock_history_events = json.loads(
        (_MOCK_FIXTURES_DIR / "history_events_tsla.json").read_text(encoding="utf-8")
    )
...
@app.get("/api/score/{ticker}/history-events")
def api_history_events(ticker: str):
    """[G8] The historical bull/bear/expansion/recession/crisis timeline, with
    this stock's own realised return, worst drawdown and volatility inside each
    event window it actually traded through.

    Historical context only: the payload carries
    `contributes_to_risk_score: false`, and nothing here is wired into the
    producer layer or `risk_score` (see market_history.py for the two reasons —
    the repo's no-weight-without-validation rule, and a survivorship-bias
    problem that rule alone would not catch).

    Fetched at period="max" rather than the card's 2y so pre-2016 events are
    measured against real prices instead of being reported as outside available
    history; it is a heavier fetch than the card's, which is why the panel
    requests it lazily on first expand rather than with the initial score.
    """
    if MOCK_MODE:
        return _mock_history_events
    try:
        raw = scorer.fetcher.fetch_history(ticker.upper(), period="max")
        close = scorer.preprocessor.process(raw)["close"]
        return overlay_events(close)
    except ValueError as exc:
        raise HTTPException(status_code=404, detail=str(exc))
    except Exception as exc:
        logger.exception(f"History events error for {ticker}: {exc}")
        raise HTTPException(status_code=500, detail="Internal error")
