...
export default function StockCard({ ticker, period, onRemove, index = 0, wide = false }) {
  const { t } = useLanguage()
  ...
  // ── Wide single-stock layout (Riscore.dc mockup): sections float as
  // separate glass panels in a two-column bento — hero + factor tiles on the
  // left, metrics/explainers/community rail on the right, charts side by
  // side across the bottom. Same components, same data, same order of
  // information — only the arrangement changes. Compare mode (2 cards) and
  // mobile keep the compact stacked card below.
  if (wide && score && !loading && !error) {
    // In the wide hero the company name carries the heading (the mockup's
        ...
        <div className="grid items-start gap-5 lg:grid-cols-[13fr_11fr]">
          ...
          <div className="space-y-5">
            <Panel delay={0} className="relative overflow-hidden">
              {/* Risk-tinted nebula wash + sweeping accent bar. */}
              <div
                aria-hidden="true"
          ...
          <div className="space-y-4">
            ...
            <Panel delay={2} hover className="overflow-hidden [&>div]:border-b-0">
              <RiskExplainer
            ...
            <Panel delay={3} hover className="overflow-hidden [&>div]:border-b-0">
              <StressTestPanel stressTest={score.stress_test} />
            </Panel>

            <Panel delay={4} hover className="overflow-hidden [&>div]:border-b-0">
              <MLSignalPanel
            ...
            <Panel delay={5} hover className="overflow-hidden">
  ...
  // Wide-mode loading/error still get a floating panel shell (no compact
  // card chrome), so the swap to the bento grid doesn't visually jump.
  if (wide) {
    return (
      <div className="animate-fade-in">
        <div className="panel overflow-hidden">
          ...
          {error && !loading && (
            <div className="animate-fade-in px-8 py-12 text-center text-sm text-down">
              ⚠ {error}
            </div>
          )}
        </div>
  ...
  // ── Compact stacked card (compare mode / mobile) — unchanged. ─────────
  return (
    <div
...
// the wide layout's shared section chrome.
