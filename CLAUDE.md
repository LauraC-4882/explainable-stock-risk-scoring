...
|---|---|---|
| `.venv/bin/python -m pytest tests/ -q` | **live** — 610 passed, 0 failed as of 2026-08-07 (was 104 on 2026-07-17) | Full test suite |
...
| `.venv/bin/python scripts/persona_smoke.py --runs 3` | **live** — landed 2026-07-24; ~90s for 3×7 sessions, exit 0 | End-to-end persona smoke: starts its own mock-mode uvicorn on a throwaway DB, drives 7 scripted user journeys (novice, power, portfolio, community, zh-TW, keyboard, mobile) through a real Chromium, and reports per-persona doability + a console-error / i18n-leak / NaN scan. Needs a built frontend (`npm run build` in `ui/web`) and Playwright browsers; not in pytest. Exits non-zero on any console/page error or crash |
